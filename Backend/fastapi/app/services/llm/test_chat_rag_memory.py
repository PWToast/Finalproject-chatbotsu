...
vector_store_from_client = Chroma(
    client=client,
    collection_name="chatbot_rag_documents",#เปลี่ยนชื่อ collectionให้ถูกต้อง
...
"""),
    ...
    return {"messages": [HumanMessage(content=rewritten_question,id=recent_question.id)],"rewritten_question": rewritten_question}
...
ตอบเป็นภาษาที่สุภาพ เข้าใจง่าย เหมาะกับนักศึกษา สามารถตอบด้วยอิโมจิได้ ห้ามตอบข้อมูลที่เกี่ยวกับโมเดล Typhoon และคุณไม่มีความเกี่ยวข้องกับ SCBX10
ห้ามขึ้นต้นคำตอบด้วย "AI:" หรือ "Assistant:" หรือ prefix ใด ๆ ให้ตอบเฉพาะข้อความเท่านั้น
ประวัติสนทนา:
{history}
"""),
        HumanMessage(content=f"คำถามล่าสุด: {user_question}") # คำถามล่าสุดของผู้ใช้
