...
function ViewDocsPage() {
  ...
  const handleSearchByAgency = async () => {
    ...
    const res = await axios.get(
      `http://localhost:8000/querybyagency/${agencyToSend}`,
      {
        headers: {
          //auth
          Authorization: `Bearer ${token}`,
  ...
  const handleSearchByText = async () => {
    ...
    const res = await axios.get(
      `http://localhost:8000/querybytext/${querytext}`,
      {
        headers: {
          //auth
          Authorization: `Bearer ${token}`,
      ...
      <div className="flex h-screen overflow-hidden bg-[#E7E9EB]">
        ...
        <main className="m-2 flex-1 flex flex-col h-full">
          <div className="flex flex-row gap-5 bg-white w-full p-3 rounded-lg mb-3 shrink-0">
            <select
              name="agency"
              className="text-xs sm:text-sm border border-gray-300 rounded p-2 bg-white w-full md:w-auto cursor-pointer"
              value={selectagency}
            ...
            ></input>
            <p
              className="text-xs sm:text-sm border border-gray-300 rounded p-2 bg-white w-full md:w-auto cursor-pointer hover:bg-gray-400 hover:text-white"
            ...
            <table className="w-full border-collapse bg-white text-center text-sm font-light text-gray-500 table-fixed">
              <thead className="bg-[#007A6D] sticky top-0 z-10">
                <tr className="text-white">
                  ...
                  <th className="w-[100px] px-3 py-3 border-r">
                    ประเภทของเนื้อหา
                  </th>
                  <th className="w-[80px] px-3 py-3 border-r">
                    หน่วยงานที่เกี่ยวข้อง
                  </th>
                  <th className="w-[50px] px-3 py-3 border-r">แหล่งที่มา</th>
                  <th className="w-[50px] px-3 py-3 border-r">
                    วันเวลาที่เพิ่ม
                  </th>
              ...
              <tbody>
                {dataToShow.map((item) => (
                  <tr
                    ...
                    className="hover:bg-gray-300 hover:text-white cursor-pointer transition-colors"
                  >
                    <td className="w-[100px] px-6 py-4 truncate">
                      {item.content}
                    </td>
                    <td className="w-[100px] px-6 py-4 truncate">
                      {item.metadata.topic}
                    ...
                    <td className="w-[80px] px-6 py-4 truncate">
                      {item.metadata.agency}
                    </td>
                    <td className="w-[50px] px-6 py-4 truncate">
                    ...
                    <td
                      className="w-[110px] px-2 py-3 cursor-pointer text-red-500 transition-all duration-200 hover:bg-red-500 hover:text-white"
                      onClick={(e) => {
                        e.stopPropagation();
        ...
        <ViewDataModal
          isOpen={showVieweModal}
          onClose={() => setShowViewModal(false)}
        >
          <h2 className="text-xl font-bold mb-2">รายละเอียดของเอกสาร</h2>
          <div className="flex flex-col">
            <p>เนื้อหา:</p>
            ...
            <p className="text-gray-600 mb-6">
              {contentToView.metadata.category}
            </p>
            <p>หน่วยงานที่เกี่ยวข้อง:</p>
            <p className="text-gray-600 mb-6">
              {contentToView.metadata.agency}
            </p>
            <p>แหล่งที่มา:</p>
            <p className="text-gray-600 mb-6">
              {contentToView.metadata.source}
            </p>
            <p>วันเวลาที่เพิ่ม:</p>
            <p className="text-gray-600 mb-6">
              {contentToView.metadata.added_at}
            </p>
          </div>
          <div className="flex justify-end gap-2">
            <button
